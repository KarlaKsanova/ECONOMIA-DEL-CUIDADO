...
def crear_base_de_datos(
    ruta: str | Path = os.path.join(ROOT_DIR, "sistema_vinculacion.db")
) -> None:
    """Crea las tablas OLTP si todavía no existen."""

    ruta_sqlite = os.fspath(ruta)
    if not os.path.isabs(ruta_sqlite):
        ruta_sqlite = os.path.join(ROOT_DIR, ruta_sqlite)
    ruta_sqlite = os.path.abspath(ruta_sqlite)
